...
class serial_recieve(object):
    def __init__(self):
        ...
        self._recieveLoop = True
        self.rxByteCount = 0

    def recieveDataLoop(self, serialclass, refreshtime):
    ...
    def recieveThreading(self, serialclass, openorstop, refreshtime):
        if openorstop == True:
            self._recieveLoop = True
            self._rThreading = threading.Thread(target=self.recieveDataLoop, args=(serialclass, refreshtime))
            self._rThreading.setDaemon(True)
